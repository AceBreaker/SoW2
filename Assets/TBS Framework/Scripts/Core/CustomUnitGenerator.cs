...
public class CustomUnitGenerator : MonoBehaviour, IUnitGenerator
{
    ...
    public void SpawnUnit(Unit unit, Cell cell)
    {
        if (!cell.IsTaken)
        {
            ...
            unit.Initialize();
            unit.unitGenerator = this;
            GetComponentInParent<CellGrid>().Units.Add(unit);

            GetComponentInParent<CellGrid>().SpawnNewUnit(unit);

            //temp.Units.FindAll(u => u.PlayerNumber.Equals(temp.CurrentPlayerNumber)).ForEach(u => { u.OnTurnStart(); });
            //temp.Players.Find(p => p.PlayerNumber.Equals(temp.CurrentPlayerNumber)).Play(temp);
        }//Unit gets snapped to the nearest cell
        else
        {
            Destroy(unit.gameObject);
